...
suffix="$(date +%s)"
external_id="http-cancel-$suffix"
...
assert_status 401 "$base_url/sales/$external_id/cancel" -X POST
assert_status 403 "$base_url/sales/$external_id/cancel" -X POST \
    -H "Authorization: Bearer $seller_token"
assert_status 404 "$base_url/sales/missing-$suffix/cancel" -X POST \
...
cancel_status="$(curl -sS -o "$tmp_dir/cancel.json" -w '%{http_code}' \
    ...
    -H "Authorization: Bearer $admin_token")"
[ "$cancel_status" = 200 ]
grep -q '"status":"canceled"' "$tmp_dir/cancel.json"
grep -q '"reversed_points":300' "$tmp_dir/cancel.json"

repeat_status="$(curl -sS -o "$tmp_dir/repeat.json" -w '%{http_code}' \
    -X POST "$base_url/sales/$external_id/cancel" \
    -H "Authorization: Bearer $admin_token")"
[ "$repeat_status" = 200 ]
grep -q '"status":"canceled"' "$tmp_dir/repeat.json"
grep -q '"reversed_points":300' "$tmp_dir/repeat.json"

printf '%s\n' 'HTTP cancellation checks passed.'
